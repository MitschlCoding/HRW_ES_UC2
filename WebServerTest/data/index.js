...
async function findAndSplitRepeats(string) {
  ...
  while (true) {
    ...
    tempStr
      ...
      .forEach((strSplit) => {
        if (strSplit != '') {
          stringBlocks.push(strSplit);
        }
      });
    var tempWhileIndex = whileIndex;
    var tempEndIndex = whileIndex;
    while (true) {
      if (
        /*tempStr.substring(tempWhileIndex + 6).search('repeat') == -1 ||*/
        tempWhileIndex > tempEndIndex ||
        tempStr.substring(tempEndIndex + 4).search('end\n') == -1
      ) {
        break;
      }
...
async function runCommand(command) {
  if (command.substring(0, 8) == 'picture(') {
    snap();
  } else if (command.substring(0, 4) == 'led(') {
  ...
  } else if (command.substring(0, 9) == 'motorULN(') {
    const deviceIdInputStart = command.search(/\(/);
    const deviceIdInputEnd = command.search(/\)/);
    const deviceIdInput = command.substring(
      deviceIdInputStart + 1,
  ...
  } else if (command.substring(0, 9) == 'motorDRV(') {
    const deviceIdInputStart = command.search(/\(/);
    const deviceIdInputEnd = command.search(/\)/);
    const deviceIdInput = command.substring(
      deviceIdInputStart + 1,
  ...
  } else if (command.substring(0, 5) == 'wait(') {
    const valueInputStart = command.search(/\(/);
    const valueInputEnd = command.search(/\)/);
    const valueInput = command.substring(valueInputStart + 1, valueInputEnd);
    await delay(parseInt(valueInput));
  } else if (command.substring(0, 7) == 'repeat(') {
    const valueInputStart = command.search(/\(/);
    const valueInputEnd = command.search(/\)/);
    var expression = command.substring(valueInputStart + 1, valueInputEnd);
    expression = expression.split(' ').map((str) => {
    ...
    while (eval(expression)) {
      await interpretCode(
        command.substring(command.search(/\)/) + 2, command.length - 4)
      );
      expression = command.substring(valueInputStart + 1, valueInputEnd);
      expression = expression.split(' ').map((str) => {
        if (stack[str] != undefined) {
          return stack[str];
        } else {
  ...
  } else if (command.substring(0, 4) == 'var ') {
    ...
    value = value.join(' ');
    console.log(value);
  } else if (command.substring(0, 3) == 'if(') {
    const valueInputStart = command.search(/\(/);
    const valueInputEnd = command.search(/\)/);
    var expression = command.substring(valueInputStart + 1, valueInputEnd);
    expression = expression.split(' ').map((str) => {
